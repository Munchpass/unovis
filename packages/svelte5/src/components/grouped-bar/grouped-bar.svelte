<script lang="ts">
  ...
  import { GroupedBar, type GroupedBarConfigInterface, type NumericAccessor } from '@unovis/ts'
  ...
  let prevConfig = $state<GroupedBarConfigInterface<Datum> | undefined>(undefined)
  const config = $derived({ x: props.x, y: props.y, ...props })

  // component declaration
  let component = $state<GroupedBar<Datum> | undefined>(undefined)
  ...
  // component accessor
  function getComponent (): GroupedBar<Datum> { return component }
