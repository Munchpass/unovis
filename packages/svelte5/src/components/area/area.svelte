<script lang="ts">
  ...
  import { Area, type AreaConfigInterface, type NumericAccessor } from '@unovis/ts'
  ...
  let prevConfig = $state<AreaConfigInterface<Datum> | undefined>(undefined)
  const config = $derived({ x: props.x, y: props.y, ...props })

  // component declaration
  let component = $state<Area<Datum> | undefined>(undefined)
  ...
  // component accessor
  function getComponent (): Area<Datum> { return component }
