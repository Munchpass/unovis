<script lang="ts">
  ...
  let prevConfig = $state<LineConfigInterface<Datum> | undefined>(undefined)
  const config = $derived({ x: props.x, y: props.y, ...props })

  // component declaration
  let component = $state<Line<Datum> | undefined>(undefined)
  ...
  // component accessor
  function getComponent (): Line<Datum> { return component }
